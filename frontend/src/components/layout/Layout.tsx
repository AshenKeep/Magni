...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-black">
      ...
      <aside className="hidden md:flex w-52 flex-col shrink-0 border-r border-border bg-surface">
      ...
      {/* ── Main content ───────────────────────────────────────────── */}
      <main className="flex-1 overflow-y-auto bg-black pb-safe-bottom md:pb-0">
        {/* Mobile header */}
        <header className="md:hidden sticky top-0 z-20 flex items-center justify-between px-4 py-3 bg-surface border-b border-border">
          <span className="text-blue font-bold text-lg tracking-tight">Magni</span>
          <span className="text-xs text-secondary">v0.0.13</span>
        </header>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 flex items-stretch bg-surface border-t border-border"
